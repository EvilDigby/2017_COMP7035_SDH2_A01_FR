...
public class ChangeMaking_2 {
								 ...
								 MyList<Integer> coinValues)
	{
		...
		//OP1. We traverse all elements in coinValues, so as to find the biggest coin not used so far. 
		while ((res == -1) && (index < size))
		{
			//OP1.1. If the coin has not been picked before, we pick it
			if (discarded.getElement(index) == 0) 
			{
				//OP1.2 If the coin is bigger than all other non discarded coins we make it the biggest coin
				if(coinValues.getElement(index) > biggestCoin) 
				{
					//We change the biggestCoin to the next biggest coin
					biggestCoin = coinValues.getElement(index);
					//We Remember the index of the coin
					biggestCoinIndex = index;
				}
		...
		res = biggestCoinIndex;

		//-----------------------------
		//Output Variable --> Return FinalValue
		//-----------------------------
		// We return the index of the biggest coin
		return res;
								   ...
								   int itemSelected)
	{
		...
		//OP1. We check if the we can use the coin or not
	 ...
	 */	
	public int solutionTest(int changeGenerated,
								MyList<Integer> discarded,
								MyList<Integer> coinValues, 
							    int amount)
	{
		...
		//OP1. We traverse all elements in coinValues.
		while (index >=0){
			//OP1.1. If the item has not been picked before
			int e0 = coinValues.getElement(index);
			if (discarded.getElement(index) == 0 && (e0 > bestCoin))
			{
				bestCoin = e0;
				res = index;
			}
	...
	public MyList<Integer> solve(MyList<Integer> coinValues, int amount)
	{
		...
		//We use 'discarded' as a MyList with the coins discared s ofar.
		...
		//OP1.1. 'res' is initialised to be a list of the size of coinValues, with all values being 0.
		res = new MyDynamicList<Integer>();
		for (int i = 0; i < size; i++)
		{
			res.addElement(0, 0);
		}
		//OP1.2. 'discarded' is initialised to be a list of the size of coinValues, with all values being 0.
		...
		while (solutionTest(changeGenerated, discarded, coinValues, amount ) != -1)
		{

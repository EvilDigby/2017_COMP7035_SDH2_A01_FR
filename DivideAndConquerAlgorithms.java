...
public class DivideAndConquerAlgorithms {
	...
	public int maxInt(MyList<Integer> m)
	{
		...
		case 2:
			...
			//2.2. We recursively go through the list
			res = maxInt(m);
	...
	public boolean isReverse(MyList<Integer> m)
	{
		...
		case 2: 
			...
			//2. If the first one is larger than the second, we keep testing the rest of MyList
			if (e0 >= e1){
				//2.1. We remove the first element from MyList
				m.removeElement(0);
			...
			//3. If the first one is smaller than the second one, we can conclude MyList is not in reverse order
			else
				res = false;
	...
	public int power(int n, int m)
	{
		...
		//Rule 1. if the power is zero we make res 1
		if (m == 0) 
			scenario = 1;
		//Rule 2. The power is greater than 1
		else
			scenario = 2;
